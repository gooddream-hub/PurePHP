...
if ( isset($_SERVER['HTTP_REFERER']) ) {
	if(strpos($_SERVER['HTTP_REFERER'], "youtube") == true ){
		...
		setAd(722);
	} elseif(strpos($_SERVER['HTTP_REFERER'], "pinterest") == true ){
		setAd(752);
	} elseif(strpos($_SERVER['HTTP_REFERER'], "twitter") == true ){
		setAd(753);
	}
}
...
function insertNews(){
	...
	$query  = "UPDATE email_stats set clicks = clicks+1 WHERE nid = $nid";
	$result = mysql_query($query);

	$query = "UPDATE email_users set last_click = NOW() WHERE uid = $eid";
	$result = mysql_query($query);	
...
function setBounce(){
	...
	if( isset( $_SERVER['HTTP_REFERER'] ) && $_SERVER['HTTP_REFERER'] == 'http://www.mjtrends.com/' && isset($_COOKIE['bounce'])){
