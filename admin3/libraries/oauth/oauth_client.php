...
class oauth_client_class
{
	...
	var $server = '';
	// var $configuration_file = '/var/www/mjtrends/admin3/libraries/oauth/oauth_configuration.json';
	...
	{
		$values = array(
			'oauth_consumer_key'=>$this->client_id, //bpcs5km3fjeshyozkh2p7azc
			'oauth_nonce'=>md5(uniqid(rand(), true)),
			'oauth_signature_method'=>$this->signature_method, //HMAC-SHA1
		...
		switch($authentication = (IsSet($options['AccessTokenAuthentication']) ? strtolower($options['AccessTokenAuthentication']) : ''))
		{
			...
			case '':
				if(strlen($authorization))
					...
					//OAuth oauth_consumer_key="bpcs5km3fjeshyozkh2p7azc",oauth_nonce="a2361bfd408a1d02927a24037e7be3ab",oauth_signature_method="HMAC-SHA1",oauth_timestamp="1608055561",oauth_version="1.0",oauth_token="3957d385437cf317fb5fa343697b63",oauth_signature="9%2FSHvnBpErYqVIS54uiVJE8bLUo%3D"
				break;
			default:
	...
	Function CallAPI($url, $method, $parameters, $options, &$response)
	{
		...
		switch(intval($this->oauth_version))
		{
			case 1:
				...
				// $oauth = Array([oauth_token] => 3957d385437cf317fb5fa343697b63)
				break;
